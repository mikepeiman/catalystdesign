<script lang="ts">
  ...
  {#if $$props.outline}
    <div class={divClass}>
      ...
      <Button2 {...$$restProps} color="none" class={gradientOutlineClass} on:click on:change on:keydown on:keyup on:mouseenter on:mouseleave><slot /></Button2>
    </div>
  {:else}
    <Button2 {...$$restProps} color="none" class={divClass} on:click on:change on:keydown on:keyup on:touchstart on:touchend on:touchcancel on:mouseenter on:mouseleave>
      <slot />
    </Button2>
  {/if}
  ...
  <style>
    ...
    .gradient-button::before {
      ...
      width: 100%;
      height: 100%;
      background-image: linear-gradient(var(--gradient-hover-angle), var(--gradient-hover-start), var(--gradient-hover-end));
      z-index: -1;
      transition: all 0.3s ease;
      opacity: 0;
    }
