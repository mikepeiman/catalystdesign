...
<div class="max-w-[100vw] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto bg-white dark:bg-gray-900">
  <div class="relative p-6 md:p-16">
    <div class="relative z-10 lg:grid lg:grid-cols-12 lg:gap-16 lg:items-center">
      <div class="mb-10 lg:mb-0 lg:col-span-6 lg:col-start-8 lg:order-2">
        <h2 class="text-2xl text-gray-800 font-bold sm:text-3xl dark:text-white mb-6">
        ...
        <nav class="grid gap-4 mt-5 md:mt-10" aria-label="Tabs" role="tablist">
          {#each services as service, index}
            <button
              type="button"
              class="group hs-tab-active:bg-white hs-tab-active:shadow-md text-start hover:bg-gray-100 p-4 md:p-5 rounded-xl dark:hs-tab-active:bg-gray-800 dark:hover:bg-gray-800 transition-all duration-200 ease-in-out {activeTab === index ? 'bg-white shadow-md dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-900'}"
            ...
            >
              <span class="flex items-center">
                <Icon icon={service.icon} class="flex-shrink-0 mt-2 size-6 md:size-7 text-gray-800 group-hover:text-blue-600 dark:text-gray-200 dark:group-hover:text-blue-400 transition-colors" />
                <span class="grow ms-6">
                  <span class="block text-lg font-semibold text-gray-800 group-hover:text-blue-600 dark:text-white dark:group-hover:text-blue-400">{service.title}</span>
                  <span class="block mt-1 text-gray-600 dark:text-gray-300">{service.description}</span>
                </span>
                <Icon icon="tabler:chevron-right" class="flex-shrink-0 size-5 text-gray-600 group-hover:text-blue-600 dark:text-gray-400 dark:group-hover:text-blue-400 transition-colors" />
              </span>
            </button>
            {#if activeTab === index}
              <div transition:slide="{{ duration: 300, easing: quintOut }}" class="pl-12">
                <p class="mt-2 text-gray-600 dark:text-gray-300">{service.expanded}</p>
          ...
          <div>
            {#each services as service, index}
              <div id="tabs-with-card-{index + 1}" role="tabpanel" aria-labelledby="tabs-with-card-item-{index + 1}" class="{activeTab === index ? '' : 'hidden'}">
                <img class="shadow-xl shadow-gray-200 rounded-xl dark:shadow-gray-800/20" src={service.image} alt={service.title}>
              </div>
            {/each}
          </div>
        </div>
